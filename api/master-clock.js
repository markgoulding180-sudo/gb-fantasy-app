...
// GET /api/master-clock - Get current gameweek
// POST /api/master-clock - Set current gameweek (admin only)
...
module.exports = async (req, res) => {
  ...
  if (req.method === 'GET') {
    try {
      ...
      if (clockError && !clockError.message.includes('0 rows')) {
        return res.status(500).json({ error: 'Failed to read master clock', details: clockError.message });
      }

      // Get FPL deadline for the next gameweek (for reference)
      ...
      try {
        ...
        console.log('FPL API fetch failed, using stored deadline');
      }

      // If no master clock exists, return default
      if (!masterClock) {
        return res.status(200).json({
          current_gameweek: 1,
          next_gameweek: 2,
          last_finalised_gameweek: 0,
          status: 'not_set',
          deadline: fplDeadline,
          deadline_epoch: fplDeadlineEpoch,
          message: 'Master clock not set - admin must initialize'
        });
      }
      ...
      return res.status(200).json({
        ...
        updated_at: masterClock.updated_at
      });
